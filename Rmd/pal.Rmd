...
-   There are three alternative fns in way more popular pkgs:
    ...
    -   `cli::ansi_collapse()`
        ...
        -   no means to use different `sep`s for length-2 and longer inputs ("oxford comma")!
        -   apart from that the most powerful/sophisticated fn!

    If the proposal in [r-lib/cli#568](https://github.com/r-lib/cli/issues/568) is gonna be implemented, retire `prose_ls()`.

    Otherwise, base it on `cli::ansi_collapse()` with and additional param `single_sep` for length-2 sep. And rename it to `collapse_chr()` in the same go!
...
-   Outsource these fns into separate pkg? Maybe first compile a systematic review of existing pkgs with similar functionality like
    [settings](https://github.com/markvanderloo/settings) and [GlobalOptions](https://github.com/jokergoo/GlobalOptions)...
...
#'
#' @param x Character vector to be converted to a Markdown list.
  ...
  kable_args <-
    alist(x = x,
          ...
          row.names = ifelse(is.null(incl_rownames),
                             NA,
                             incl_rownames),
          col.names = colnames(x) %>% when(checkmate::assert_flag(strong_colnames) ~ paste0("**", ., "**"),
                                           ~ .),
          label = label,
...
-   There's already the [tinkr](https://docs.ropensci.org/tinkr/) package providing [`to_xml()`](https://docs.ropensci.org/tinkr/reference/to_xml.html). The
    ...
    -   replace *certain* typographic single and double quotes with the basic commonmark-compliant ones via a call to . I think it is a pretty bad idea to
...
-   Unfortunately, the `gh_*`functions below are [officially out of scope](https://github.com/r-lib/gh/issues/173) for the underlying
    [gh](https://gh.r-lib.org/) package.
...
jsonvalidate::json_validate(
    ...
    schema = "/home/salim/Code/own/salim_b/r/pkgs/qstnr/inst/json-schema/qstnr-survey.schema.json",
    engine = "ajv",
...
-   rename to `sort_by()` to avoid confusion with [`dplyr::order_by()`](https://dplyr.tidyverse.org/reference/order_by.html)?
