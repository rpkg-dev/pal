...
## Import unexported functions from other packages

Cf. <https://stackoverflow.com/a/46098814/7196903>
...
#'
#' @param parse_emoji_markup Should emoji markup (e.g. `:smile:`) be parsed as Unicode emoticons? Setting this to `TRUE` activates
#'   [Pandoc's `emoji` extension](https://pandoc.org/MANUAL.html#extension-emoji).
#' @param smart_punctuation If `TRUE`, straight quotes are converted to curly quotes, `---` to an em-dash (—), `--` to an en-dash (–), and `...` to ellipses (…). Nonbreaking spaces are inserted after certain abbreviations, such as `Mr.`.
...
Code largely borrowed from `rlang::arg_match()`.
