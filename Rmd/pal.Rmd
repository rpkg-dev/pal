...
as_flat_list <- function(x,
  ...
  if (regard_attrs || depth < 2L) {
    ...
    result <- x %>% purrr::when(keep_attrs ~ rm_list_lvl(.,
    ...
    result <-
      ...
      purrr::map(.f = as_flat_list,
                 ...
                 attrs_to_drop = attrs_to_drop) %>%
      purrr::when(keep_attrs ~ rm_list_lvl(.,
                                           attrs_to_drop = attrs_to_drop),
                  ~ purrr::flatten(.))
  }
...
-   Check for primitive functions using `is.primitive()` and properly handle that case (we can't determine `parent_param_names` for them, so we should probably
    just abort with an informative error). Also mention that in docs and adapt examples.

-   Thoroughly check examples, there seem to be flaws in the fn!
...
build_readme <- function(input = "README.Rmd",
  ...
  if (!isFALSE(build_index_md)) {
    ...
    if (is_pkgdown_dir(output_dir)) {
      ...
      ## TODO: as soon as knitr version 1.37 is released that [fixes this](https://github.com/yihui/knitr/pull/2059), require it and use `fs::file_temp()`
      tmp_file <-
        fs::file_temp(pattern = "tmp-pal-build_readme",
                      tmp_dir = parent_dir,
...
#' create files which are in turn included in another HTML file, e.g. via a static site generator.
#' 
#' The HTML widget's dependencies must be written to files separately, e.g. using [write_widget_deps()].
#'
