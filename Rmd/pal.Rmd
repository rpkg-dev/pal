...
### Normalize a tree path

Useful for GitHub's GraphQL API.
...
reduce_df_list <- function(x,
  ...
  if (tibble::is_tibble(x) | is.data.frame(x)) {
    ...
    if (checkmate::assert_flag(strict)) {
      rlang::abort(cli::format_error("At least one element of the list to be reduced is not a data frame / tibble!"))
...
knitr_table_format <- function(default = c("pipe",
  ...
  if (!(result %in% allowed_formats)) {
    ...
    rlang::abort(cli::format_error(paste0("R option {.field knitr.table.format} must evaluate to one of ",
...
assert_pkg <- function(pkg,
  ...
  if (!is_pkg_installed(pkg = checkmate::assert_string(pkg),
    ...
    rlang::abort(cli::format_error(paste(message, install_hint)))
...
-   [ ] Use `desc::desc_get_field()`'s `squish_ws` param as soon as the next stable version containing [\#101](https://github.com/r-lib/desc/pull/101) is
    released. And rename fn to something like `desc_get_field_safe()`?
...
-   [ ] Propose this function in upstream.
...
is_pkg_installed <- function(pkg,
  ...
  if (is.null(min_version)) {
    ...
    if (is.null(min_version[[1L]])) {
      rlang::abort(cli::format_error("{arg min_version} must be a vector of package versions or something coercible to: ", min_version[[2L]]))
    }
...
path_script <- function() {
  ...
  rlang::abort(cli::format_error("Couldn't determine script path."))
}
```
...
prose_ls_fn_param <- function(param,
  ...
  if (is.primitive(fn)) rlang::abort(cli::format_error("Listing parameters of R Primitives is not supported. Sorry."))
  ...
  if (param %in% names(default_vals)) {
    ...
    fn_name <- deparse1(expr = substitute(fn),
                        backtick = FALSE)
    rlang::abort(cli::format_error("The function {.fn {fn_name}}` does not have a parameter named {.arg {param}}."))
  }
  
  if (missing(default_vals)) {
    fn_name <- deparse1(expr = substitute(fn),
                        backtick = FALSE)
    rlang::abort(cli::format_error("{.fn {fn_name}}'s parameter {.arg {param}} does not have a default value."))
  }
...
is_http_success <- function(url,
  ...
  rlang::with_handlers(!httr::http_error(httr::RETRY(verb = "HEAD",
                                                     ...
                                                     quiet = quiet)),
                       error = ~ FALSE,
                       interrupt = ~ rlang::abort(cli::format_error("Terminated by the user")))
}
```
...
#' @param .action The action to take when the check fails. A function expecting an error message string as first argument. For example
#'   [`rlang::abort`][rlang::abort()], [`rlang::warn`][rlang::warn()], [`rlang::inform`][rlang::inform()] or [`rlang::signal`][rlang::signal()].
#' @export
#'
#' @examples
...
cols_regex <- function(...,
  ...
  if (length(setdiff(names(patterns), "")) < length(patterns)) {
    rlang::abort(cli::format_error("All column specifications in {.arg ...} must be named by a regular expression."))
  }
