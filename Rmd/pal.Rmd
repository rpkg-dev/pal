...
```{r}
# `bg_grey()` missing from packages cli/crayon
bg_grey <- crayon::make_style("darkslategrey",
                              bg = TRUE)
```
...
-   Find out why param [`df_print`](https://bookdown.org/yihui/rmarkdown/html-document.html#data-frame-printing) has no effect (seems to always fall back to `"default"`; the same happens for `rmarkdown::github_document()`). Is this a bug?\`
...
There is a similar (but way simpler) function [`pkgsearch:::needs_packages()`](https://github.com/r-hub/pkgsearch/blob/9c0102e85929f25d23cb4f497ba4f01c3d3c0cd9/R/utils.R#L100-L127) and [I've asked](https://github.com/r-hub/pkgsearch/issues/102) if there are any plans on standardizing this.

TODO:

-   Incorporate code from FA.

-   Add additional params `install_source`. Open question: How to best handle multiple pkgs at once if they don't share the install source? Just recycle `install_source`?
...
-   The "real" data in the examples stems from [here](https://opendata.swiss/en/dataset/wahlarchiv-des-kantons-zuerich/resource/893a7b0e-e394-4874-a0af-a99fb39358fd).
              ...
              .f = function(path) {
                ...
                if (process_line_by_line) {
                  ...
                  readr::read_lines(path) %>%
                    purrr::map_chr(.f = str_replace_all_verbose,
                                   pattern = pattern,
                                   verbose = verbose,
                                   cli_id = path) %>%
                    readr::write_lines(path = path,
                                       na = NA_character_)
                } else {
                  
                  readr::read_file(path) %>%
                    str_replace_all_verbose(pattern = pattern,
                                            verbose = verbose,
                                            cli_id = path) %>%
...
str_replace_all_verbose <- function(string,
                                    pattern,
                                    verbose,
                                    cli_id = NULL) {
  
  checkmate::assert_string(string)
  checkmate::assert_string(cli_id,
                           null.ok = !verbose)
  
  # print replacement info for humans
  if (verbose) {
    
    cli::cli_ul(id = cli_id,
                .auto_close = FALSE)
    ...
    for (i in seq_along(pattern)) {
      ...
      purrr::walk2(.x = hit[, "start"],
                   ...
                   .f = function(start, end) {
                     ...
                     # print info
                     cli::cli_li(items = as_string("Replaced ", bg_grey(excerpt_begin), cli::bg_magenta(pattern_asis), bg_grey(excerpt_end)),
                                 id = paste0(cli_id, "-li"),
                                 .auto_close = FALSE)
                     cli::cli_text(as_string(rep("\u00A0", 4L), "with ",
                                             bg_grey(excerpt_begin), cli::bg_blue(pattern[i]),bg_grey(excerpt_end)))
                     cli::cli_end(id = paste0(cli_id, "-li"))
                   })
      
      string %<>% stringr::str_replace_all(pattern = pattern[i])
    }
    
    cli::cli_end(id = cli_id)
    return(string)
