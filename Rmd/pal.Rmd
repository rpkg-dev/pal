...
safe_max <- function(...,
  ...
  for (i in seq_along(input)) {
    checkmate::assert_numeric(input[[i]],
                              typed.missing = TRUE,
                              null.ok = TRUE,
                              .var.name = "...")
  }
...
-   Once <https://github.com/tidyverse/purrr/pull/1179> is merged and released, switch from interim `list_flatten()` to `purrr::list_flatten()`.
...
#' @param x A numeric vector to phrase.
...
use_pkg <- function(package,
  ...
  # TODO: remove this once https://github.com/r-lib/usethis/pull/2117 has been merged and released
...
#'   pal::exists_in_namespace("funky_config", "pkgpurl")
#' }
#'
#' pal::exists_in_namespace("funky_config", "pal")
...
-   [Upstream](https://github.com/r-lib/desc/issues/162) this function?
...
Complementing and extending the [**roxygen2**](https://roxygen2.r-lib.org/) package.
...
-   R \< 4.5.0 only allowed plaintext as `\link` text in `.Rd` files. roxygen2 optionally allows *the whole* link text to be formatted as inline code (via a
    simple hack). So it basically wasn't allowed to mix text both formatted as inline code and not inside documentation link text (roxygen2 used to [throw a
    warning](https://github.com/r-lib/roxygen2/blob/a34bdf7c7b952068b1f7742b7b34d1eac78e54da/R/markdown-link.R#L107) if tried). This simplified link text
    replacing quite a bit for us.

    Revisit whether changes are needed after [r-lib/roxygen2#1712](https://github.com/r-lib/roxygen2/pull/1712) is merged and released!

NOTES:

-   Stripping all valid MD links using `xml2::xml_text(pal::md_to_xml(x))` so that only roxygen2 documentation links remain won't work since the Markdown
...
-   Fix failing examples during `pkgdown::build_site()` or -- if not possible to fix -- figure out how to disable running these examples during pkgdown build.
...
-   Systematically compare with `xfun::md_table()` and consider upstreaming `pipe_table()`'s current upsides (col/rownames formatting etc.) there.
...
-   There are two noteworthy R packages for R Markdown file parsing:
    ...
    1.  [**parsermd**](https://rundel.github.io/parsermd/) which uses custom C++ code under the hood and therefore should be very fast. But it appears to
        implement its [own custom abstract syntax tree (AST)](https://rundel.github.io/parsermd/articles/parsermd.html#working-with-the-ast), which seems like a
        silly idea.

        We should thoroughly check out the package and compare it to the XML-based tinkr approach!

    2.  [**tinkr**](https://docs.ropensci.org/tinkr/), whose built-in XSL stylesheet we already use (via `tinkr::stylesheet()` and `tinkr::md_ns()`).

        tinkr also provides its own [`to_xml()`](https://docs.ropensci.org/tinkr/reference/to_xml.html). The main advantage of `pal::md_to_xml()` is that
        `pal::strip_yaml_header()` is more robust in YAML header detection than [blogdown's
        ...
        2.  Submit PR replacing [`blogdown:::split_yaml_body()`](https://github.com/rstudio/blogdown/blob/main/R/utils.R#L761-L775) with
            `xfun::split_yaml_body()`.

        3.  Submit PR replacing [`xaringan:::split_yaml_body()`](https://github.com/yihui/xaringan/blob/master/R/utils.R#L34-L43) with
            `xfun::split_yaml_body()`.
        ...
        -   parse knitr *in-header* chunk options via a call to `tinkr:::parse_rmd()`, something `pal::md_to_xml()` simply omits (but which could be very useful
            for certain use cases; note though that it doesn't yet [support *in-body* chunk options](https://github.com/ropensci/tinkr/issues/55)).
...
-   Move this function to pkg gitlab?
...
Extending the [**checkmate**](https://mllg.github.io/checkmate/) package.
...
Extending the [**gert**](https://docs.ropensci.org/gert/) and [**git2r**](https://docs.ropensci.org/git2r/) packages.

### `git_file_mod_time`

NOTES:

-   [Using Git's `log` subcommand](https://stackoverflow.com/a/56373126/7196903) might be more efficient than `blame` we use here. But since
    [`gert::git_log()`](https://docs.ropensci.org/gert/reference/git_commit.html) doesn't allow to specify a file path and git2r doesn't offer a `log` wrapper
    function, we don't really have a choice.
...
Extending the [**httr2**](https://httr2.r-lib.org/) package which is built upon the [curl](https://jeroen.cran.dev/curl/) package.
...
-   Switch to [`tombi` CLI](https://tombi-toml.github.io/tombi/docs/cli)! Note that it has a different [config
    approach](https://tombi-toml.github.io/tombi/docs/configuration).

NOTES:

-   An alternative to the Rust-based external `taplo` CLI would be to rely on the R package [jsonvalidate](https://docs.ropensci.org/jsonvalidate/) that wraps
    the JavaScript tool [Ajv](https://ajv.js.org/). The **main drawback** is that it doesn't feature native TOML support, meaning only JSON files can be
    directly validated. So we'd need to convert the TOML to JSON before validation -- which a) isn't possible in a strictly lossless manner due to the
    additional native date(time) types formats TOML supports as well as [serialization
    ambuigity](https://docs.ropensci.org/jsonvalidate/reference/json_serialise.html#details) when the conversion is done via an intermediary R representation
    (`RcppTOML::parseTOML()` can only convert TOML to R lists, not directly to JSON, but R doesn't have a native atomic type, only vectors, thus the
    "atomicness" information is lost when converting R lists to JSON), and b) could fail for an invalid TOML file. After successful conversion, validation
    errors would then refer to the JSON representation which obviously is less useful. Also, Ajv is notably slower than Taplo (which is built in Rust).
    ...
    (the highest version [`taplo` currently supports](https://taplo.tamasfe.dev/configuration/developing-schemas.html)). *Update: `tombi` would also have that
    advantage without any additional drawbacks!*
...
-   Our PR that would fix `rprojroot::find_root()` got [rejected](https://github.com/r-lib/rprojroot/pull/100#issuecomment-2439864749) in the end for
    questionable reasons. But we'd need it to be able to really *rely* on `rprojroot::find_root()` as used below. Discuss or try with another PR?

-   Submit this function as `check_criterion()` upstream as outlined [from here on](https://github.com/r-lib/rprojroot/pull/83#issuecomment-1874876315).
