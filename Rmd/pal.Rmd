...
- Switch to using `knitr::kable()`; we would have to rely on `magrittr::set_rownames(paste("**", ., "**"))` to implement `strong_colnames`.
...
#' Convert to a character scalar (aka string)
#'
#' This function is like [`paste0(..., collapse = TRUE)`][paste0()], but _recursively_ converts its inputs to type character.
#'
...
```r
is_http_success <- purrr::possibly(~ httr::http_status(httr::HEAD(.x))$category == "Success",
                                   otherwise = FALSE)
```

TODO: Propose this fn [upstream](https://github.com/r-lib/httr), maybe renamed (`is_online()`?).
