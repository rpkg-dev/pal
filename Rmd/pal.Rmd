        ...
        if (is_pkgdown_dir(output_dir)) {
          ...
          brio::read_file(input) %>%
            ...
            stringr::str_replace_all(pattern = "(<img [^>]+)(height=['\"].*?['\"]\\s*)",
                                     replacement = "\\1") %>%
            # remove Rmd chunk with label 'pkgdown'
            stringr::str_replace_all(pattern = "\\n```\\{ *(engine *= *)?([rR]|['\"][rR]['\"]) *, *(label *= *)?['\"]?pkgdown['\" ,][^\\}]*?\\}(\\n|.)*?\\n```",
                                     replacement = "") %>%
            brio::write_file(path = index_md_path)
          
          # knit
          knitr::knit(input = index_md_path,
                      output = index_md_path,
                      quiet = TRUE,
                      envir = env)
          
          # render `pkgdown/index.md`
          rmarkdown::render(input = index_md_path,
