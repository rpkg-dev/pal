...
-   Add support for system keyrings as (4th) pkg config val src via [`keyring::key_set_with_value()`](https://keyring.r-lib.org/reference/key_get.html), which
    is more secure to store secrets than the existing srcs.
...
-   Outsource these fns into separate pkg?
    ...
    -   **pkgopt**
    -   pkgset
    -   pkgstate (but state is rather misleading and evokes associations with [actual state](https://r-pkgs.org/data.html#sec-data-state))
...
<!-- From here on up to but excluding *System interaction*, <h2>-level topics are supposed to be arranged alphabetically. -->
...
TODO: When we finished switching to httr2, deprecate this fn in favor of
[`httr2::resp_check_content_type()`](https://httr2.r-lib.org/reference/resp_check_content_type.html).
...
-   A shorter way to define a very similar function would be:
    ...
    ``` r
    is_http_success <- purrr::possibly(\(x) !httr::http_error(x),
                                       otherwise = FALSE)
    ```
